...
class Matrix:
    """
    Класс для хранения двумерных числовых матриц на основе списков.
    Матрицу должно быть возможным создать из списка (в этом случае у неё будет одна строка),
    списка списков, или же передав явно три числа: число строк, число столбцов и значение
    по-умолчанию (которое можно не задавать, в этом случае оно принимается равным нулю).
    """
    ...
    def track_matrix(self):
        ...
        :return: Возвращает след матрицы.
        """
        if self.n != self.m:
            ...
            for row in enumerate(self.rows):
                for i in range(len(row[1])):
                    if i == row[0]:
                        tr.append(row[1][i])
            print('rwertwert-terwterwtete', sum(tr))
            return sum(tr)
...
if __name__ == "__main__":
    ...
    print("Матрица из столбцов, строк и значания по умолчанию",b)
    c = Matrix([1, 5, 7])
